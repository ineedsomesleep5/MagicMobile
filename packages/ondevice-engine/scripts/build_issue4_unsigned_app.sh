...
# XcodeGen intentionally updates the tracked project from the reviewed YAML.
# Pin those exact generated bytes; all non-generated source must stay unchanged.
python3 "$ROOT/scripts/verify_issue4_unsigned_product.py" --repo "$REPO" \
  --record-generated-project --output "$OUT/generated-project.json"
...
python3 "$ROOT/scripts/verify_issue4_unsigned_product.py" --app "$APP" --repo "$REPO" --project-record "$OUT/generated-project.json" --output "$OUT/product-receipt.json"
