...
- `XMAGE_SMOKE_SCENARIO=prompt-mode` is a targeted mode-choice probe. The first June 23, 2026 run used real XMage and `directStateSeeded: true`, but Austere Command surfaced as `GAME_CHOOSE_ABILITY:ability`, so the current fixture uses Lavabrink Venturer's upstream `ChooseModeEffect` instead.
