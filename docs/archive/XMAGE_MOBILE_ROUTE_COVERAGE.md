...
| mulligan | Opening hand boolean response; `mulligan` | Yes; sends `sendPlayerBoolean(true)` | Yes | Yes | Yes | Yes, action tray | Yes, `repeated-mulligan` | Yes | deterministic fixture-proven | Keep repeated-mulligan fixture current. |
...
| choose mana | Mana choice prompt / bridge response kind `mana` | Yes for `choose_mana`, but `GAME_PLAY_MANA` maps primarily to `play_mana` | Yes | Yes | Yes | Yes, mana choice picker | Yes, `choose-mana` | Yes | deterministic fixture-proven | Keep choose-mana fixture current. |
...
| card prompt | `GAME_SELECT` / card list prompt | Yes; emits `choose_card` or `search_select` | Yes | Yes | Yes | Yes, card picker | Yes, `choose-card` | Yes | deterministic fixture-proven | Keep choose-card fixture current. |
| player prompt | `GAME_TARGET` with player UUID | Yes; emits player options / `choose_player` | Yes | Yes | Yes | Yes, player option grid | Yes, `choose-player` | Yes | deterministic fixture-proven | Keep choose-player fixture current. |
...
| amount prompt | `GAME_GET_AMOUNT` / `GAME_PLAY_XMANA` | Yes; emits `choose_amount` / `play_x_mana` | Yes | Yes | Yes | Yes, amount picker | Yes, targeted `prompt-amount` fixture for `GAME_GET_AMOUNT` and `play-x-mana` | Yes | deterministic fixture-proven | Keep play_x_mana and choose_amount fixtures current. |
...
| commander replacement | `GAME_ASK` mentioning commander/command zone | Yes; emits `commander_replacement` | Yes | Yes | Yes | Yes, command-zone/original-zone buttons | Yes, gauntlet/full aggregate artifact | Yes in targeted gauntlet evidence | deterministic fixture-proven | The older full-AI artifact predates the expanded aggregate; rerun all 21 scenarios before release evidence is cited. |
...
| replacement prompts | Generic replacement-choice prompts | Yes; generic replacement distinct via `generic_replacement` | Yes | Yes for those shapes | Yes for those shapes | Partial | Yes, `generic-replacement` | Yes | deterministic fixture-proven | Keep generic replacement prompt classifier current. |
...
| graveyard | `PlayerView.getGraveyard()` | Yes; snapshot and `xmage.players[].zones.graveyard` | Yes | Yes | N/A | Yes, surface chip/zone sheet | Yes, `zone-movement` | Yes | deterministic fixture-proven | Keep zone movement fixture current. |
| exile | `PlayerView.getExile()` / named exile zones | Yes; player exile and named `exileZones` | Yes | Yes | N/A | Yes, surface chip/zone sheet | Yes, `zone-movement` | Yes | deterministic fixture-proven | Keep zone movement fixture current. |
...
`commander-full-ai` runs 21 required deterministic scenarios and reports `allRequiredScenariosPassed`, `routeFamiliesCovered`, `routeFamiliesMissing`, `scenarioRouteFamiliesMissing`, `stepsBlocked`, `iOSRequiredRoutesMissing`, and `readinessVerdict`. In addition to the original aggregate scenarios, it now requires repeated mulligan, choose-card, choose-player, X-mana, mana/color choice, generic replacement, zone movement, opening starting-player choice, and the phone drag-cast regression. A route seen in one child cannot hide the same route missing from another required child.

The fixture route is dev/test-only and production-disabled. The old default bridge remains a remote XMage `Session` client, but fixture mode now launches an embedded same-JVM `MagicMobileEmbeddedServerBridge` so the route can reach server-owned `GameController` / `Game.cheat(...)`. Older commander-gauntlet and 12-scenario full-AI artifacts passed with real gameplay snapshots, but they predate the 21-scenario aggregate and are not current release proof. Rerun the expanded aggregate on the final checkout before treating it as green.
...
The June 24, 2026 `commander-full-ai` artifact used real `source: "xmage-java-bridge"` evidence with `directStateSeeded: true` and `seededStateVerified: true`, but covered the former 12-scenario inventory. The gate-only self-test now verifies all 21 required scenario omissions and every declared per-scenario route omission fail closed; fresh live bridge evidence for the expanded inventory is still required before release.
