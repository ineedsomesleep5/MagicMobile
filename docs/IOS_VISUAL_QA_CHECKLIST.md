...
- `iphone-17-pro-max-action-dock-priority-first-landscape-readable.jpg`: refreshed board proof after the right dock was reordered so prompt/pass/primary actions appear before secondary zone surfaces.
- `iphone-17-pro-max-missing-art-placeholders-landscape-readable.jpg`: readable board proof from a real fixture snapshot launched in Debug with `MAGICMOBILE_FORCE_CARD_PLACEHOLDERS=true`; card art URLs are intentionally disabled on the client and parchment placeholders hold stable card dimensions.
...
- `build_output/screenshots/ios-action-dock.png`: current right-dock checkpoint showing prompt/pass controls before secondary surfaces.
- `build_output/screenshots/ios-missing-art.png`: current missing-art checkpoint using a real XMage fixture board and the simulator-only forced-placeholder image toggle.
...
- The right dock now orders prompt/action controls before zone chips and exposes count-only Library plus Revealed/Looked sheets when real XMage data is present; recapture active-prompt screenshots to verify the dock still reads cleanly.
...
- The next live fixture screenshot should verify the new Arena-inspired tabletop background, centered life medallions, and narrower right dock with real `source: xmage-java-bridge` data. The local Next web API was intentionally stopped after the current pass to free RAM, so recapture requires a brief web API restart or a device build pointed at an active web API.
