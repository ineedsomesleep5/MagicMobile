...
- Do not mark real iPhone alpha ready yet. The backend gauntlet is green and commander damage is separately proven, but real blocker assignment and prompt-variety remain later-scope unless explicitly moved into the alpha gate, and real iPhone manual QA is still unchecked.
...
| Commander damage | Targeted `commander-damage` fixture is live-proven on real XMage with non-empty commander damage matrix and AI life change. | Keep damage matrix tied to XMage rules text; do not fabricate if missing. | small | `MagicMobileBridge.java`, smoke runner, iOS/web HUDs | Render commander damage more prominently in iOS/web and keep the targeted smoke current. |
| Commander Gauntlet | A real `XMAGE_SMOKE_SCENARIO=commander-gauntlet` gate now exists with deterministic server-side fixture setup and reports completed/blocked steps from the real bridge. Latest run passed with `directStateSeeded: true`, `seededStateVerified: true`, and `stepsBlocked: []`. | Keep the passing full acceptance loop current: mana, fetch/search, commander cast, stack/trigger/ability, commander removal/replacement, recast tax, and AI continuation. | small | `apps/xmage-gateway/scripts/smoke-create-commander-game.ts`, `MagicMobileBridge.java`, docs | Keep fixture gauntlet in validation; do not substitute non-fixtured smoke as proof. |
