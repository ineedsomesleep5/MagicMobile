...
- Prompt-variety is not green until real XMage proof covers `stack_object_seen`, `activate_ability`, `choose_ability`, `choose_mode`, `order_triggers/order_items`, `choose_amount`, `choose_multi_amount`, and `choose_pile`. The `choose_mode` slice is now deterministic-fixture proven by `prompt-mode` on game `22c1aafb-41f4-4d8c-a8c8-914b64a19a12` with `source: "xmage-java-bridge"`, `directStateSeeded: true`, `seededStateVerified: true`, final `bridgeRevision: 51`, final `xmageCycle: 91`, `actionsByType.choose_mode: 1`, `promptMode.choiceSubmitted: true`, `promptMode.choiceResolved: true`, and empty `stepsBlocked`. The standalone `prompt-variety` scenario remains red; the next targeted slice should be order triggers/items, likely with a Soul Warden plus Spirited Companion fixture.
...
6. **Full-AI fixture expansion**: Add targeted deterministic prompt scenarios for `choose_amount`, `choose_multi_amount`, `choose_pile`, `order_triggers/order_items`, and damage assignment. Suggested representative fixture cards are `Wheel of Misfortune`, `Manamorphose`, `Fact or Fiction`, and a Soul Warden plus Spirited Companion trigger-order setup, using a Commander-legal five-color fixture shell when needed.
...
4. Full Commander vs AI is not ready until `commander-full-ai` is green, or the product explicitly excludes and safely falls back for every missing prompt/damage-assignment family. Mana-rock, Arcane Signet as a generic mana-rock regression, commander damage, blocker-flow, activated-stack, triggered-stack, and prompt-mode already have targeted real-XMage fixture proof, but full prompt-variety and damage assignment do not.
