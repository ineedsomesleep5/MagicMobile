...
- Full Commander vs AI now has a stricter aggregate smoke target: `ENABLE_XMAGE_FIXTURES=true NODE_ENV=test XMAGE_GATEWAY_URL=http://localhost:17171 XMAGE_SMOKE_SCENARIO=commander-full-ai XMAGE_USE_FIXTURE=true pnpm smoke:xmage`. The latest automated route gate passed with `readinessVerdict: "full-commander-vs-ai-ready"`, no missing route families, no blocked steps, and no iOS-required routes missing. Do not use `commander-gauntlet` alone to claim full Commander parity.
