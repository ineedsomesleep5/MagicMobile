...
| `ENABLE_XMAGE_FIXTURES=true NODE_ENV=test XMAGE_GATEWAY_URL=http://localhost:17171 XMAGE_SMOKE_SCENARIO=commander-full-ai XMAGE_USE_FIXTURE=true pnpm smoke:xmage` | Full Commander vs AI aggregate gate | Fresh local pass before iOS-only UI changes | The June 24/25, 2026 local rerun wrote `build_output/smoke/smoke-report-commander-full-ai.json` with real `source: "xmage-java-bridge"`, `directStateSeeded: true`, `seededStateVerified: true`, `allRequiredScenariosPassed: true`, `routeFamiliesMissing: []`, `iOSRequiredRoutesMissing: []`, `stepsBlocked: []`, and `readinessVerdict: "full-commander-vs-ai-ready"`. No backend or fixture code changed after this rerun in the Magic Path/iOS UI pass. |
...
- Full Commander vs AI now has a stricter aggregate smoke target: `ENABLE_XMAGE_FIXTURES=true NODE_ENV=test XMAGE_GATEWAY_URL=http://localhost:17171 XMAGE_SMOKE_SCENARIO=commander-full-ai XMAGE_USE_FIXTURE=true pnpm smoke:xmage`. The current local artifact at `build_output/smoke/smoke-report-commander-full-ai.json` reports `readinessVerdict: "full-commander-vs-ai-ready"` with no missing route families or blocked steps. It was freshly rerun before this iOS-only UI pass; do not use targeted child scenario passes alone to claim full Commander parity.
...
6. **Full-AI fixture upkeep**: Keep the currently green local `commander-full-ai` aggregate fresh whenever bridge, fixture, prompt, or iOS command models change, and rerun it on the final checkout before release evidence is cited.
