...
- Full Commander vs AI now has a stricter aggregate smoke target: `ENABLE_XMAGE_FIXTURES=true NODE_ENV=test XMAGE_GATEWAY_URL=http://localhost:17171 XMAGE_SMOKE_SCENARIO=commander-full-ai XMAGE_USE_FIXTURE=true pnpm smoke:xmage`. The latest run failed honestly with `readinessVerdict: "not-ready-full-commander-vs-ai"` after the first four child scenarios passed, because repeated fixture-created games in one XMage process began returning snapshots whose command zone never materialized (`human.commandZone missing Isamaru, Hound of Konda` or `Kenrith, the Returned King`). Do not use `commander-gauntlet` alone to claim full Commander parity.
- `damage-assignment` is an active full-AI blocker. A deterministic probe scenario exists, but the bridge/shared/Swift/iOS route is not implemented and no real XMage `damage_assignment` prompt has been proven. Current status is bridge missing / fixture missing, not later scope. The current `commander-full-ai` run also exposes a fixture lifecycle blocker before reaching a valid damage-assignment proof: later child fixture games fail seeded proof because command-zone commanders are missing.
...
6. **Full-AI fixture expansion**: Add deterministic proof for damage assignment and fix the back-to-back fixture-game lifecycle so command zones remain present across the full aggregate; standalone aggregate `prompt-variety` is now reconciled and green.
...
4. Full Commander vs AI is not ready until `commander-full-ai` is green, or the product explicitly excludes and safely falls back for every missing prompt/damage-assignment family. Mana-rock, Arcane Signet as a generic mana-rock regression, commander damage, blocker-flow, activated-stack, triggered-stack, and standalone aggregate prompt-variety already have real-XMage fixture proof, but the full aggregate currently fails on repeated fixture-game command-zone proof and damage assignment still has no route proof.
