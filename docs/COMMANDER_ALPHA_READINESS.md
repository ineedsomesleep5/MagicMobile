...
- Focused blocker-flow fixture evidence now uses a Commander-legal colorless `Memnite` attacker in the Kozilek AI deck. The latest pass used game `8437628b-6dcf-419c-96a6-7ceee659e138`, `source: "xmage-java-bridge"`, `directStateSeeded: true`, final `bridgeRevision: 10`, final `xmageCycle: 14`, `blockerAssignmentExercised: true`, and `stepsBlocked: []`.
...
- `laterScope` remains non-empty in the gauntlet report for `mana-rock`, `commander-damage`, `blocker-flow`, and `prompt-variety`; targeted mana-rock, commander-damage, blocker-flow, activated-stack, and triggered-stack are separately deterministic-fixture proven. Prompt-variety is still not green.
...
- `commander-damage`, `blocker-flow`, and `prompt-variety` are still `laterScope` in the latest gauntlet report. Targeted mana-rock and commander damage are separately proven. Targeted `blocker-flow` is now separately proven on game `8437628b-6dcf-419c-96a6-7ceee659e138` with `source: "xmage-java-bridge"`, `directStateSeeded: true`, `blockerAssignmentExercised: true`, final `bridgeRevision: 10`, final `xmageCycle: 14`, and `stepsBlocked: []`. Do not claim later-scope routes as part of the gauntlet gate unless fresh reports move them into the required gate and prove them with real XMage.
- Prompt-variety is not green until the aggregate route-family gate reconciles the targeted real-XMage proofs for `stack_object_seen`, `activate_ability`, `choose_ability`, `choose_mode`, `order_triggers/order_items`, `choose_amount`, `choose_multi_amount`, and `choose_pile`. The `choose_mode` slice is deterministic-fixture proven by `prompt-mode` on game `22c1aafb-41f4-4d8c-a8c8-914b64a19a12`. The `order_triggers/order_items` slice is deterministic-fixture proven by `prompt-order` on game `d8763ace-f8fc-412b-8537-f93ba2c02207` with `source: "xmage-java-bridge"`, `directStateSeeded: true`, `seededStateVerified: true`, final `bridgeRevision: 29`, final `xmageCycle: 50`, `actionsByType.order_items: 1`, `promptOrder.choiceSubmitted: true`, and empty `stepsBlocked`. The `choose_amount` slice is deterministic-fixture proven by `prompt-amount` on game `da1e4c2e-533e-4cb5-987f-d1ff76cd0aca` with `source: "xmage-java-bridge"`, `directStateSeeded: true`, `seededStateVerified: true`, final `bridgeRevision: 18`, final `xmageCycle: 24`, `actionsByType.choose_amount: 1`, `promptAmount.choiceSubmitted: true`, and empty `stepsBlocked`. The `choose_multi_amount` slice is deterministic-fixture proven by `prompt-multi-amount` on game `75488774-6919-4003-b77f-d4e3371dd48d` with `source: "xmage-java-bridge"`, `directStateSeeded: true`, `seededStateVerified: true`, final `bridgeRevision: 18`, final `xmageCycle: 29`, `actionsByType.choose_multi_amount: 1`, `promptMultiAmount.choiceResolved: true`, and empty `stepsBlocked`. The `choose_pile` slice is deterministic-fixture proven by `prompt-pile` on game `bfca4936-50c7-432d-9c06-28c937243a9d` with `source: "xmage-java-bridge"`, `directStateSeeded: true`, `seededStateVerified: true`, final `bridgeRevision: 20`, final `xmageCycle: 33`, `actionsByType.choose_pile: 1`, `promptPile.choiceResolved: true`, and empty `stepsBlocked`.
- Full Commander vs AI now has a stricter aggregate smoke target: `ENABLE_XMAGE_FIXTURES=true NODE_ENV=test XMAGE_GATEWAY_URL=http://localhost:17171 XMAGE_SMOKE_SCENARIO=commander-full-ai XMAGE_USE_FIXTURE=true pnpm smoke:xmage`. This command is expected to fail until `prompt-variety` and `damage-assignment` are implemented or formally excluded from the full release. Do not use `commander-gauntlet` alone to claim full Commander parity.
- `damage-assignment` is an active full-AI blocker. A deterministic probe scenario exists, but the bridge/shared/Swift/iOS route is not implemented and no real XMage `damage_assignment` prompt has been proven. Current status is bridge missing / fixture missing, not later scope.
...
6. **Full-AI fixture expansion**: Add deterministic proof for damage assignment and reconcile `prompt-variety` as an aggregate of the targeted prompt slices.
...
4. Full Commander vs AI is not ready until `commander-full-ai` is green, or the product explicitly excludes and safely falls back for every missing prompt/damage-assignment family. Mana-rock, Arcane Signet as a generic mana-rock regression, commander damage, blocker-flow, activated-stack, triggered-stack, prompt-mode, prompt-order, prompt-amount, prompt-multi-amount, and prompt-pile already have targeted real-XMage fixture proof, but aggregate prompt-variety and damage assignment do not.
