...
- Prompt-variety is not green until real XMage proof covers `stack_object_seen`, `activate_ability`, `choose_ability`, `choose_mode`, `order_triggers/order_items`, `choose_amount`, `choose_multi_amount`, and `choose_pile`. Current `prompt-variety` is fixture-gated and honest: it seeds real XMage state, but reports `stepsBlocked: ["prompt-variety"]` with missing `activate_ability`, `choose_ability`, `choose_mode`, `order_triggers/order_items`, `choose_amount`, `choose_multi_amount`, and `choose_pile`. The next targeted slice should be order triggers/items, likely with a Soul Warden plus Spirited Companion fixture.
...
- `prompt-mode` has been added as the first targeted prompt-variety slice. It uses Austere Command in a legal mono-white Commander fixture, and the latest real smoke proved direct seeding but failed the route gate: XMage surfaced the choice as `GAME_CHOOSE_ABILITY:ability`, not `choose_mode`. The next fix is either a different representative card/state that emits `GAME_CHOOSE_CHOICE` mode data or an evidence-backed bridge classification rule that does not relabel ordinary ability choices.
...
6. **Full-AI fixture expansion**: Add targeted deterministic prompt scenarios for `choose_mode`, `choose_amount`, `choose_multi_amount`, `choose_pile`, `order_triggers/order_items`, and damage assignment. Suggested representative fixture cards are `Austere Command`, `Wheel of Misfortune`, `Manamorphose`, `Fact or Fiction`, and a Soul Warden plus Spirited Companion trigger-order setup, using a Commander-legal five-color fixture shell.
...
4. Full Commander vs AI is not ready until `commander-full-ai` is green, or the product explicitly excludes and safely falls back for every missing prompt/damage-assignment family. Mana-rock, Arcane Signet as a generic mana-rock regression, commander damage, blocker-flow, activated-stack, and triggered-stack already have targeted real-XMage fixture proof, but full prompt-variety and damage assignment do not.
