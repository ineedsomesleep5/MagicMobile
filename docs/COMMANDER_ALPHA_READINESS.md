...
6. **Later-scope fixture expansion**: Continue targeted deterministic scenarios for prompt-variety once those are explicitly in alpha scope. Mana-rock is now targeted-fixture proven with Sol Ring; Arcane Signet remains a separate manual/debug regression. Blocker assignment now has targeted fixture proof, but it is still outside the current gauntlet gate.
...
4. Decide whether `prompt-variety` must move from `laterScope` into the iPhone alpha gate. Mana-rock, Arcane Signet as a generic mana-rock regression, commander damage, blocker-flow, activated-stack, and triggered-stack already have targeted real-XMage fixture proof, but full prompt-variety does not.
