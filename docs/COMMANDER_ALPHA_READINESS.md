...
| XcodeBuildMCP card-target simulator QA | Hand selection and primary action visibility | Pass | The refreshed simulator build exposed card targets such as `card-hand-sol-ring-<id>`. Automated QA selected Sol Ring from hand, preserved the battlefield layout, and showed the primary Cast action. Screenshot: `build_output/screenshots/ios-hand-selected.png`. Missing-art screenshot recapture remains pending because the live fixture's visible cards loaded art from the local image API. |
