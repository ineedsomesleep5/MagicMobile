...
- Targeted `mana-rock` now uses a Commander-legal singleton `Sol Ring` fixture and is live-proven through real XMage snapshots. The separate Arcane Signet two-lands manual regression is still not proven fixed; earlier Arcane evidence returned to priority with the card still in hand/castable after `Cast Arcane Signet`.
...
- `commander-damage`, `blocker-flow`, and `prompt-variety` are still `laterScope` in the latest gauntlet report. Targeted mana-rock and commander damage are separately proven. Targeted `blocker-flow` is now separately proven on game `51ea0b22-dbdc-45cf-850d-d11c3d5329a6` with `source: "xmage-java-bridge"`, `directStateSeeded: true`, `blockerAssignmentExercised: true`, final `bridgeRevision: 11`, final `xmageCycle: 14`, and `stepsBlocked: []`. Do not claim later-scope routes as part of the gauntlet gate unless fresh reports move them into the required gate and prove them with real XMage.
...
4. Decide whether `blocker-flow` or `prompt-variety` must move from `laterScope` into the iPhone alpha gate. Mana-rock, commander damage, and blocker-flow already have targeted real-XMage fixture proof, but they are not part of the current gauntlet gate. If Arcane Signet is treated as an alpha-specific regression, rerun the mana-rock fixture with `XMAGE_SMOKE_MANA_ROCK_CARD="Arcane Signet"` and trace the cast/payment response until it opens payment or moves the card instead of returning an unchanged priority snapshot.
