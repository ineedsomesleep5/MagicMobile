...
| `ENABLE_XMAGE_FIXTURES=true NODE_ENV=test XMAGE_GATEWAY_URL=http://localhost:17171 XMAGE_SMOKE_SCENARIO=commander-full-ai XMAGE_USE_FIXTURE=true pnpm smoke:xmage` | Full Commander vs AI aggregate gate | Fails after child smokes | Current honest failure: real `source: "xmage-java-bridge"`, `directStateSeeded: true`, `seededStateVerified: true`, all child scenarios except `damage-assignment` passed; final verdict `not-ready-full-commander-vs-ai`, missing `damage_assignment`. |
...
- Final deterministic smoke evidence from this pass used `source: "xmage-java-bridge"`, direct fixture seeding, seeded-state verification, final `bridgeRevision: 133`, final `xmageCycle: 219`, and `stepsBlocked: []`.
...
- `commander-damage`, `blocker-flow`, and `prompt-variety` are still `laterScope` in the latest gauntlet report, but each now has separate targeted real-XMage fixture proof. Targeted `blocker-flow` is proven on game `8437628b-6dcf-419c-96a6-7ceee659e138` with `source: "xmage-java-bridge"`, `directStateSeeded: true`, `blockerAssignmentExercised: true`, final `bridgeRevision: 10`, final `xmageCycle: 14`, and `stepsBlocked: []`. Do not claim later-scope routes as part of the gauntlet gate unless fresh reports move them into the required gate and prove them with real XMage.
...
- Full Commander vs AI now has a stricter aggregate smoke target: `ENABLE_XMAGE_FIXTURES=true NODE_ENV=test XMAGE_GATEWAY_URL=http://localhost:17171 XMAGE_SMOKE_SCENARIO=commander-full-ai XMAGE_USE_FIXTURE=true pnpm smoke:xmage`. The latest run failed honestly with `readinessVerdict: "not-ready-full-commander-vs-ai"` only because `damage-assignment` did not prove `damage_assignment`. Do not use `commander-gauntlet` alone to claim full Commander parity.
- `damage-assignment` is an active full-AI blocker. A deterministic probe scenario exists, but the bridge/shared command route is not implemented and no real XMage `damage_assignment` prompt has been proven. iOS now has a named safe fallback that refuses to auto-submit a damage split, but current route status remains bridge missing / fixture missing, not later scope.
...
6. **Full-AI fixture expansion**: Add deterministic proof for damage assignment; standalone aggregate `prompt-variety` is now reconciled and green.
...
4. Full Commander vs AI is not ready until `commander-full-ai` is green, or the product explicitly excludes and safely falls back for every missing prompt/damage-assignment family. Mana-rock, Arcane Signet as a generic mana-rock regression, commander damage, blocker-flow, activated-stack, triggered-stack, and prompt-variety already have real-XMage fixture proof, but damage assignment still has no route proof.
