...
- `prompt-mode` has been added as the first targeted prompt-variety slice. The first Austere Command attempt proved direct seeding but failed the route gate because XMage surfaced the modal spell as `GAME_CHOOSE_ABILITY:ability`, not `choose_mode`. The current probe uses Lavabrink Venturer in a legal mono-white Commander fixture because upstream XMage implements it with `ChooseModeEffect`.
...
6. **Full-AI fixture expansion**: Add targeted deterministic prompt scenarios for `choose_mode`, `choose_amount`, `choose_multi_amount`, `choose_pile`, `order_triggers/order_items`, and damage assignment. Suggested representative fixture cards are `Lavabrink Venturer`, `Wheel of Misfortune`, `Manamorphose`, `Fact or Fiction`, and a Soul Warden plus Spirited Companion trigger-order setup, using a Commander-legal five-color fixture shell when needed.
