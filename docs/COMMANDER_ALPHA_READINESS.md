...
| `ENABLE_XMAGE_FIXTURES=true NODE_ENV=test XMAGE_GATEWAY_URL=http://localhost:17171 XMAGE_SMOKE_SCENARIO=mana-rock XMAGE_USE_FIXTURE=true pnpm smoke:xmage` | Targeted Arcane Signet payment-source smoke | Fails fast after services ready | Current honest failure: real `source: "xmage-java-bridge"`, direct fixture seeding, failed at `mana-rock-cast-no-progress`, final observed `bridgeRevision: 10`, final observed `xmageCycle: 11` |
...
- Targeted `mana-rock` now uses a Commander-legal singleton fixture deck and direct server-side seeding, but it is not live-proven. The current failure is precise: after a real `Cast Arcane Signet` action, XMage returns to priority with Arcane Signet still in hand/castable and no payment prompt, zone movement, stack object, or mana change.
...
6. **Later-scope fixture expansion**: Continue targeted deterministic scenarios for mana-rock activation and prompt-variety once those are explicitly in alpha scope. The mana-rock fixture is now legal and fail-fast, but still blocked at `mana-rock-cast-no-progress`. Blocker assignment now has targeted fixture proof, but it is still outside the current gauntlet gate.
...
4. Decide whether `mana-rock`, `blocker-flow`, or `prompt-variety` must move from `laterScope` into the iPhone alpha gate. Commander damage and blocker-flow already have targeted real-XMage fixture proof, but they are not part of the current gauntlet gate. If `mana-rock` is moved into the gate, the next bridge fix is to trace the XMage card-click/cast response for Arcane Signet until it opens payment or moves the card instead of returning an unchanged priority snapshot.
