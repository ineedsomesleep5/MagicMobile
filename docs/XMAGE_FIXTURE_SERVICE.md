...
- `prompt-variety`: still missing deterministic real-XMage proof for `choose_pile`. `prompt-mode` has deterministic real-XMage proof through a targeted Lavabrink Venturer fixture for XMage's upstream `ChooseModeEffect`; `prompt-order` has deterministic real-XMage proof through Soul Warden plus Spirited Companion and XMage's target-style triggered-ability ordering prompt; `prompt-amount` has deterministic real-XMage proof through Wheel of Misfortune's upstream `GAME_GET_AMOUNT` callback; `prompt-multi-amount` has deterministic real-XMage proof through Manamorphose's upstream `GAME_GET_MULTI_AMOUNT` callback. The earlier Austere Command attempt was intentionally left as a non-proof because XMage surfaced it as `GAME_CHOOSE_ABILITY:ability`.
...
Representative fixture candidate for the next prompt-variety slice is `Fact or Fiction` for piles. Use a Commander-legal fixture shell, such as a five-color commander when needed, and keep production routing generic.
