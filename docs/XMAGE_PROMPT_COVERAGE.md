...
`prompt-variety` is a route-family proof, not a count of any three prompts. It requires real XMage evidence for `stack_object_seen`, `activate_ability`, `choose_ability`, `choose_mode`, `order_triggers/order_items`, `choose_amount`, `choose_multi_amount`, and `choose_pile`. The standalone aggregate `prompt-variety` scenario still needs reconciliation with the targeted slices; `prompt-mode`, `prompt-order`, `prompt-amount`, `prompt-multi-amount`, and `prompt-pile` are targeted prompt-variety slices and are now part of the stricter `commander-full-ai` gate. Do not describe full-product readiness until the aggregate gate proves every required prompt family or explicitly excludes it with a safe iOS fallback.
