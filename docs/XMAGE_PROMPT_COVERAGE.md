...
The latest targeted `prompt-order` proof on June 24, 2026 used game `d8763ace-f8fc-412b-8537-f93ba2c02207`, real `source: "xmage-java-bridge"`, `fixtureCallUsed: true`, `directStateSeeded: true`, `seededStateVerified: true`, final `bridgeRevision: 29`, final `xmageCycle: 50`, `promptFamiliesSeen: ["GAME_PLAY_MANA:mana", "GAME_TARGET:order"]`, `actionsByType.order_items: 1`, `promptOrder.choiceSubmitted: true`, `routeFamiliesMissing: []`, and `stepsBlocked: []`. This proves the generic trigger/item ordering route with Soul Warden plus Spirited Companion; it does not prove amount, multi-amount, pile, or damage-assignment prompts.
...
`prompt-variety` is a route-family proof, not a count of any three prompts. It requires real XMage evidence for `stack_object_seen`, `activate_ability`, `choose_ability`, `choose_mode`, `order_triggers/order_items`, `choose_amount`, `choose_multi_amount`, and `choose_pile`. The standalone `prompt-variety` scenario remains red; `prompt-mode` and `prompt-order` are targeted prompt-variety slices and are now part of the stricter `commander-full-ai` gate. Do not describe full-product readiness until targeted fixture reports prove every required prompt family or explicitly exclude it with a safe iOS fallback.
