...
| **Damage Assignment** | `damage_assignment` | No | No | No | Active full-AI blocker. A probe scenario exists, but bridge emission/command handling, shared types, command models, and real fixture proof are still missing. iOS has a named safe fallback that refuses to submit a default split. |
...
`damage-assignment` is also part of the stricter full-AI gate. The current probe starts from a deterministic combat fixture, but no inspected bridge path emits or accepts a `damage_assignment` command yet. A passing blocker-flow or commander-damage report does not prove manual damage assignment. The iOS app now shows a dedicated unsupported damage-assignment fallback and does not submit a default split, but full parity is not ready until that route is implemented/proven or explicitly excluded from the product target.
