...
| **Choose Mode / Ability** | `choose_mode`, `choose_ability` | Yes | Yes | Partial | `choose_ability` is deterministic-fixture proven through Terramorphic Expanse and `activated-ability-stack`; `choose_mode` still needs targeted fixture proof. iOS command construction for both is unit-tested through `PromptCommandBuilder`. |
