...
| **Choose Mode / Ability** | `choose_mode`, `choose_ability` | Yes | Yes | Partial | `choose_ability` is deterministic-fixture proven through Terramorphic Expanse, `activated-ability-stack`, and the failed Austere Command probe. `choose_mode` now has a targeted `prompt-mode` probe using Lavabrink Venturer's upstream `ChooseModeEffect`; it is not marked green until that real smoke passes. iOS command construction for both is unit-tested through `PromptCommandBuilder`. |
...
`prompt-variety` is a route-family proof, not a count of any three prompts. It requires real XMage evidence for `stack_object_seen`, `activate_ability`, `choose_ability`, `choose_mode`, `order_triggers/order_items`, `choose_amount`, `choose_multi_amount`, and `choose_pile`. The current fixture-gated `prompt-variety` run used real `source: "xmage-java-bridge"` and `directStateSeeded: true`, but still reported `stepsBlocked: ["prompt-variety"]` and missing `activate_ability`, `choose_ability`, `choose_mode`, `order_triggers/order_items`, `choose_amount`, `choose_multi_amount`, and `choose_pile`. `prompt-mode` is the first targeted prompt-variety slice and is now part of the stricter `commander-full-ai` gate. The first Austere Command attempt stayed red because XMage exposed that modal spell through `GAME_CHOOSE_ABILITY:ability`; the current probe uses Lavabrink Venturer's upstream `ChooseModeEffect` instead. Do not describe full-product readiness until targeted fixture reports prove all prompt families.
