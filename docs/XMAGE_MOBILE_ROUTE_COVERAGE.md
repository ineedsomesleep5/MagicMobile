...
| damage assignment | Combat damage assignment prompt/routes | No dedicated callback found; likely generic `chooseUse` + `getAmount` for `ControllerDivideCombatDamageAbility` cards | No | No | No | Named safe fallback; refuses to submit a default split | Probe only; no successful fixture proof | No | fixture/bridge classification missing | Build a targeted Butcher Orgg-style fixture, classify the resulting generic XMage prompts as damage assignment metadata if proven, and keep full-AI red until the real path passes. |
...
`commander-full-ai` runs the required deterministic scenarios and reports `allRequiredScenariosPassed`, `routeFamiliesCovered`, `routeFamiliesMissing`, `stepsBlocked`, `iOSRequiredRoutesMissing`, and `readinessVerdict`. It is expected to fail until `damage-assignment` has real XMage proof or is explicitly excluded with safe iOS fallback behavior.

The `commander-gauntlet` gate passed locally on June 24, 2026. The stricter `commander-full-ai` aggregate gate is not green yet because `damage-assignment` is still incomplete. The fixture route is dev/test-only and production-disabled. The old default bridge remains a remote XMage `Session` client, but fixture mode now launches an embedded same-JVM `MagicMobileEmbeddedServerBridge` so the route can reach server-owned `GameController` / `Game.cheat(...)`. The latest successful commander-gauntlet child report used real gameplay snapshots, `source: "xmage-java-bridge"`, `fixtureCallUsed: true`, `directStateSeeded: true`, `seededStateVerified: true`, final `bridgeRevision: 133`, final `xmageCycle: 219`, and `stepsBlocked: []`.
...
Targeted live smoke has now proven additional pieces of the route chain, including mana-rock, Arcane Signet as a generic mana-rock regression, activated-ability stack, triggered-ability stack, commander damage, blocker assignment, prompt mode, prompt order, prompt amount, prompt multi-amount, prompt pile, and aggregate prompt-variety. The current deterministic gauntlet still leaves `mana-rock`, `commander-damage`, `blocker-flow`, and `prompt-variety` in `laterScope`; treat targeted and aggregate reports as separate deterministic fixture proof, not as evidence that the narrower gauntlet release gate requires those routes. For full Commander vs AI, the aggregate `commander-full-ai` gate is the intended source of truth and must fail while `damage-assignment` is missing.

Latest `commander-full-ai` aggregate evidence on June 24, 2026 used real `source: "xmage-java-bridge"`, `directStateSeeded: true`, `seededStateVerified: true`, and passed child scenarios for `commander-gauntlet`, `mana-rock`, `commander-damage`, `blocker-flow`, `activated-ability-stack`, `triggered-ability-stack`, `prompt-mode`, `prompt-order`, `prompt-amount`, `prompt-multi-amount`, and `prompt-pile`. It failed closed with `readinessVerdict: "not-ready-full-commander-vs-ai"`, `routeFamiliesMissing: ["damage_assignment"]`, and `stepsBlocked: ["damage-assignment:route-family:damage_assignment", "route_family:damage_assignment", "scenario:damage-assignment"]`.
