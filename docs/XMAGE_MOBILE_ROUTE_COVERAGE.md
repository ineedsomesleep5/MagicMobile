...
`commander-full-ai` runs the required deterministic scenarios and reports `allRequiredScenariosPassed`, `routeFamiliesCovered`, `routeFamiliesMissing`, `stepsBlocked`, `iOSRequiredRoutesMissing`, and `readinessVerdict`. It is expected to fail until the remaining prompt-variety gaps (`choose_multi_amount` and `choose_pile`) plus `damage-assignment` have real XMage proof or are explicitly excluded with safe iOS fallback behavior.
...
Targeted live smoke has now proven additional pieces of the route chain, including mana-rock, Arcane Signet as a generic mana-rock regression, activated-ability stack, triggered-ability stack, commander damage, blocker assignment, prompt mode, prompt order, and prompt amount. The current deterministic gauntlet still leaves `mana-rock`, `commander-damage`, `blocker-flow`, and `prompt-variety` in `laterScope`; treat targeted reports as separate deterministic fixture proof, not as evidence that the gauntlet release gate requires those routes. For full Commander vs AI, the aggregate `commander-full-ai` gate is the intended source of truth and must fail while `prompt-variety` or `damage-assignment` is missing.
