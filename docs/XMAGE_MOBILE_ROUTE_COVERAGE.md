...
Latest `commander-full-ai` aggregate evidence on June 24, 2026 used real `source: "xmage-java-bridge"`, `directStateSeeded: true`, `seededStateVerified: true`, and passed child scenarios for `commander-gauntlet`, `mana-rock`, `commander-damage`, `blocker-flow`, `activated-ability-stack`, `triggered-ability-stack`, `prompt-mode`, `prompt-order`, `prompt-amount`, `prompt-multi-amount`, `prompt-pile`, and `damage-assignment`. It reported `allRequiredScenariosPassed: true`, `routeFamiliesMissing: []`, `stepsBlocked: []`, `iOSRequiredRoutesMissing: []`, and `readinessVerdict: "full-commander-vs-ai-ready"`. This is automated route proof only; real iPhone manual QA is still required before calling the product release complete.
