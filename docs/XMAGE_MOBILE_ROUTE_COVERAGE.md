...
| mode prompt | `GAME_CHOOSE_CHOICE` message containing mode | Yes; emits `choose_mode` | Yes | Yes | Yes | Yes, option grid | Targeted `prompt-mode` fixture probe exists | No | fixture missing | Current probe uses Lavabrink Venturer's upstream `ChooseModeEffect`; prove with real XMage before marking deterministic-fixture-proven. |
