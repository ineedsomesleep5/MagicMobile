...
| trigger stack | Triggered ability object/order prompt | Generic stack/order can carry it; `triggered-ability-stack` scenario is prepared | Yes for stack and order routes | Partial; see stack metadata note | Order submit partial | Stack renders generic object; trigger ordering UI partial | Targeted scenario exists | No | fixture missing | Add deterministic ETB/trigger proof using the existing fixture service. |
...
Targeted live smoke has now proven additional pieces of the route chain, including activated-ability stack and commander damage. The current deterministic gauntlet still leaves `mana-rock`, `commander-damage`, `blocker-flow`, and `prompt-variety` in `laterScope`; treat the targeted commander-damage report as separate deterministic fixture proof, not as evidence that the gauntlet release gate requires that route.
