...
Targeted live smoke has now proven additional pieces of the route chain, including mana-rock, Arcane Signet as a generic mana-rock regression, activated-ability stack, triggered-ability stack, commander damage, and blocker assignment. The current deterministic gauntlet still leaves `mana-rock`, `commander-damage`, `blocker-flow`, and `prompt-variety` in `laterScope`; treat targeted reports as separate deterministic fixture proof, not as evidence that the gauntlet release gate requires those routes.
