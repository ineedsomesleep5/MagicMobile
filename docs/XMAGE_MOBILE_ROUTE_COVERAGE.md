...
| make mana | Playable mana source/action | Yes; emits `make_mana`, adds source mana during payment, sends source UUID | Yes | Yes | Yes | Yes, battlefield tap and payment source buttons | Yes, core-flow smoke | No | complete | Add mana-rock proof after fixture service exists. |
...
| order triggers | `GAME_CHOOSE_CHOICE` order/stack prompt | Yes; maps order response to `order_items`; shared also has `order_triggers` | Yes | Yes | Yes | Partial; submit-shown-order only | No | No | UI missing | Add reorderable trigger UI and fixture. |
| order items | `GAME_CHOOSE_CHOICE` order/stack prompt | Yes; emits `orderedItems` / `order_items` | Yes | Yes | Yes | Partial; submit-shown-order only | No | No | UI missing | Add reorderable item UI and fixture. |
...
| declare attackers | Combat step `declare-attackers` | Yes; emits attacker/defender pair payloads | Yes | Yes | Yes | Partial; action button submits pair, no full combat assignment UI | Yes, combat smoke historically | No | wired but unproven | Reprove current bridge and improve combat UI. |
| declare blockers | Combat step `declare-blockers` | Yes; emits blocker/attacker pair payloads | Yes | Yes | Yes | Partial; generic action button disabled for multi-choice, no blocker picker | Scenario exists | No | UI missing | Add blocker picker plus deterministic blocker fixture. |
| damage assignment | Combat damage assignment prompt/routes | No generic route found | No | No | No | No | No | No | bridge missing | Map XMage damage assignment callbacks/actions when encountered. |
...
| stack objects | `GameView.getStack()` | Yes; emits `xmage.stack`, player stack zone, source id/name, rules text, paid status, and controller/targets only when XMage exposes them | Yes | Partial; existing stack fields decode, optional controller/target metadata still needs Swift model widening | N/A | Yes, stack surface shows count/top/source/respond-pass/pending prompt | Yes historically after cast | No | wired but unproven | Reprove current stack evidence; widen Swift model for controller/targets. |
| activated ability stack | Activated ability object on stack | Generic stack can carry it; `activated-ability-stack` scenario is prepared | Yes as stack/action types | Partial; see stack metadata note | Ability submit yes | Stack renders generic object | Targeted scenario exists | No | blocked by fixture service | Add in-server fixture setup for Loran/non-mana ability proof. |
| trigger stack | Triggered ability object/order prompt | Generic stack/order can carry it; `triggered-ability-stack` scenario is prepared | Yes for stack and order routes | Partial; see stack metadata note | Order submit partial | Stack renders generic object; trigger ordering UI partial | Targeted scenario exists | No | blocked by fixture service | Add in-server fixture setup for ETB trigger proof. |
...
| commander damage | Parsed from commander rules text/log variants | Yes; emits `commanderDamage` map | Yes | Yes | N/A | Partial; decoded but not prominent UI | Historical only; latest gauntlet leaves this in `laterScope` | No | later scope | Add targeted unblocked commander combat fixture when needed. |
...
Targeted deterministic stack proof commands are prepared but remain later scope until their fixture states force the required stack/prompt families:
...
Historical live smoke has proven additional pieces of the route chain, including combat attackers/blockers and commander damage. The current deterministic gauntlet leaves `mana-rock`, `commander-damage`, `blocker-flow`, and `prompt-variety` in `laterScope`; do not present those as current release-gate successes unless a fresh real-XMage report moves them into the required gate.
