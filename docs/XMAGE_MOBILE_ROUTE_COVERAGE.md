...
| make mana | Playable mana source/action | Yes; emits `make_mana`, adds source mana during payment, sends source UUID | Yes | Yes | Yes | Yes, battlefield tap and payment source buttons | Yes, core-flow smoke and blocked `mana-rock` fixture | No for mana-rock payment-source proof | complete for basic mana; mana-rock proof blocked | Investigate why `Cast Arcane Signet` returns to priority with the card still in hand instead of opening payment/zone/stack progress. |
...
Targeted live smoke has now proven additional pieces of the route chain, including activated-ability stack, commander damage, and blocker assignment. The current deterministic gauntlet still leaves `mana-rock`, `commander-damage`, `blocker-flow`, and `prompt-variety` in `laterScope`; treat targeted commander-damage and blocker-flow reports as separate deterministic fixture proof, not as evidence that the gauntlet release gate requires those routes.
...
Latest targeted mana-rock proof attempt on June 23, 2026 used game `689d1a70-69dd-4b04-8e52-0de6cb7e205a`, `source: "xmage-java-bridge"`, `fixtureCallUsed: true`, and `directStateSeeded: true`, but failed intentionally at `mana-rock-cast-no-progress`. The fixture is now Commander-legal (`Arcane Signet` x1, `Plains` x98); the remaining blocker is that a real `Cast Arcane Signet` action returns to priority with Arcane Signet still in hand/castable and no payment prompt, stack object, zone move, or mana change. Do not mark mana-rock activation/payment-source proof as green until that route progresses through XMage.
