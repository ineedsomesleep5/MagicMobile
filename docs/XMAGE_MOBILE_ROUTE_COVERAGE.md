...
| commander replacement | `GAME_ASK` mentioning commander/command zone | Yes; emits `commander_replacement` | Yes | Yes | Yes | Yes, command-zone/original-zone buttons | Yes, gauntlet/full aggregate artifact | Yes in current local artifact | deterministic fixture-proven | `build_output/smoke/smoke-report-commander-full-ai.json` currently reports `routeFamiliesMissing: []` and `iOSRequiredRoutesMissing: []`; rerun the aggregate on the final checkout before release evidence is cited. |
...
Latest local `commander-full-ai` aggregate artifact on June 24, 2026 uses real `source: "xmage-java-bridge"` evidence with `directStateSeeded: true` and `seededStateVerified: true`. The generated report at `build_output/smoke/smoke-report-commander-full-ai.json` currently reports `allRequiredScenariosPassed: true`, `routeFamiliesMissing: []`, `stepsBlocked: []`, `iOSRequiredRoutesMissing: []`, and `readinessVerdict: "full-commander-vs-ai-ready"`. This is local artifact evidence only; rerun the aggregate on the final checkout and complete real iPhone manual QA before release.
