...
`commander-full-ai` runs the required deterministic scenarios and reports `allRequiredScenariosPassed`, `routeFamiliesCovered`, `routeFamiliesMissing`, `stepsBlocked`, `iOSRequiredRoutesMissing`, and `readinessVerdict`. It is expected to fail until repeated fixture-created games keep command-zone proof stable across the full aggregate, and until `damage-assignment` has real XMage proof or is explicitly excluded with safe iOS fallback behavior.

The `commander-gauntlet` gate passed locally on June 23, 2026. The stricter `commander-full-ai` aggregate gate is not green yet because repeated fixture-created games eventually fail command-zone seed proof, and `damage-assignment` is still incomplete. The fixture route is dev/test-only and production-disabled. The old default bridge remains a remote XMage `Session` client, but fixture mode now launches an embedded same-JVM `MagicMobileEmbeddedServerBridge` so the route can reach server-owned `GameController` / `Game.cheat(...)`. The latest successful commander-gauntlet report used game `1f47d3b4-6e7a-43ac-9440-f1923a41c1b1` with real gameplay snapshots, `source: "xmage-java-bridge"`, `fixtureCallUsed: true`, `directStateSeeded: true`, `seededStateVerified: true`, final `bridgeRevision: 133`, final `xmageCycle: 223`, and `stepsBlocked: []`.
