...
| multi amount prompt | `GAME_GET_MULTI_AMOUNT` | Yes; emits `choose_multi_amount` with per-slot `multiAmounts` metadata | Yes | Yes | Yes | Safe fallback only; iOS can decode slot labels/ranges but still needs full touch controls | Yes, targeted `prompt-multi-amount` fixture | Yes | deterministic fixture-proven; UI missing | Build real multi-amount UI with one control per slot. |
