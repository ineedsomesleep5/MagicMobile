...
| declare blockers | Combat step `declare-blockers` | Yes; emits blocker/attacker pair payloads | Yes | Yes | Yes | Partial; generic action button disabled for multi-choice, no blocker picker | Scenario exists but now fails unless `declare_blockers` is actually submitted | No | UI missing | Add blocker picker plus deterministic blocker fixture. |
