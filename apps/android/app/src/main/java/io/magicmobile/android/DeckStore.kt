...
data class SavedDeck(
    ...
    /** The public deck link an import came from (Deck Studio shows it in the import receipt). */
...
class DeckStore(context: Context) {
    ...
    @Synchronized fun save(deck: Deck, original: SavedDeck? = null, sourceURL: String? = original?.sourceURL): SavedDeck {
        ...
        val result=SavedDeck(id,revision,deck,original?.favorite ?: false,original?.tags.orEmpty(),original?.notes.orEmpty(),sourceURL=sourceURL?.take(2048))
    ...
    @Synchronized fun duplicate(record: SavedDeck, name: String = "${record.deck.name} copy"): SavedDeck {
        ...
        val result=SavedDeck(UUID.randomUUID().toString(),1,current.deck.copy(name=name),false,current.tags,current.notes,sourceURL=current.sourceURL)
    ...
    private fun decodeSaved(value: Obj,expectedID:String): SavedDeck {
        ...
        val source=(value["sourceURL"] as? String)?.takeIf { it.length<=2048 }
        return SavedDeck(id,revision,Deck.decode(Wire.objectValue(value["deck"])),favorite,organization.tags,organization.notes,sourceURL=source)
    }
    private fun SavedDeck.json(): Obj = mapOf("schema" to 2,"id" to id,"revision" to revision,"deck" to deck.json(),"favorite" to favorite,"tags" to tags,"notes" to notes) +
        (sourceURL?.let { mapOf("sourceURL" to it) } ?: emptyMap())
